...
def gaussian(database_name):
    ...
    params = [
        ...
        [["Como a estatistica calculada para a curtose (", "{kurtosis}", ") é um valor menor do que o valor crítico inferior (", "{lower}", "), temos evidências para rejeitar a hipoótese nula, e podemos dizer que a distribuição não apresenta curtose similar a curtose de uma distribuição Normal (com", "{100*(1-alfa)}", "% de confiança)."], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ###########################
    ...
    params = [
        ...
        [["KurtosisResult", "Statistics", "Critical", "Alpha"], position, fk_id_function, 2, cursor, connection, fk_id_contributor], # en
        [["ResultadoCurtose", "Estatistica", "Critico", "Alfa" ], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ###########################
    ...
    params = [
        [["Text"], position, fk_id_function, 1, cursor, connection, fk_id_contributor], # univ
        [["T" ], position, fk_id_function, 2, cursor, connection, fk_id_contributor], # en
        [["T", ], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ###########################
    ...
    #     [["Como o p-valor (", "{p_value}", ") é menor do que o nível de significancia adotado (", "{alfa}", ") , temos evidências para rejeitar a hipoótese nula, e podemos dizer que a distribuição não apresenta curtose similar a curtose de uma distribuição Normal (com", "{100*(1-alfa)}", "% de confiança)"], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    # ]
    # for param in params:
    #     func.insert_into_message_message_parts(*param)
    # ###########################
    ...
    #     [["Como o p-valor (", "{p_value}", ") é maior do que o nível de significancia adotado (", "{alfa}", ") , não temos evidências para rejeitar a hipoótese nula, e podemos dizer que a distribuição apresenta curtose similar a curtose de uma distribuição Normal (com", "{100*(1-alfa)}", "% de confiança)"], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    # ]
    # for param in params:
    #     func.insert_into_message_message_parts(*param)
    # ###########################
