...
def draw_lilliefors_tabulated_values(database_name):
    ...
    func_name = 'draw_lilliefors_tabulated_values'
    params = [
        (func_name,),
    ]
    cursor.executemany("""
    ...
    ###########################################################
    ############ draw_lilliefors_tabulated_values #############
    ###########################################################
    ...
    params = [
        [['Text',], position, fk_id_function, 1, cursor, connection, fk_id_contributor], # univ
        [['Tabulated values of the Lilliefors test',], position, fk_id_function, 2, cursor, connection, fk_id_contributor], # en
        [['Valores tabelados do teste de normalidade de Lilliefors',], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ###########################
