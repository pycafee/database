...
def shapiro_wilk(database_name):
    ...
    params = [
        ...
        [['Since the tabulated value (', "{tabulated}", ") <= statistic (", "{statistic}", "), we have NO evidence to reject the hypothesis of data normality, according to the Shapiro-Wilk test at a", "{100*(1-alfa)}", "% of confidence level."], position, fk_id_function, 2, cursor, connection, fk_id_contributor], # en
        [['Como o valor tabelado (', "{p_value}", ") <= estatística (", "{statistic}", "), nós NÃO temos evidências para rejeitar a hipótese de normalidade dos dados, de acordo com o teste de Shapiro-Wilk com", "{100*(1-alfa)}", "% de confiança."], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ...
    params = [
        [['Text', "{tabulated}", "Text", "{statistic}", "Text", "{100*(1-alfa)}", "Text"], position, fk_id_function, 1, cursor, connection, fk_id_contributor], # univ
        [['Since the tabulated value (', "{tabulated}", ") > statistic (", "{statistic}", "), we HAVE evidence to reject the hypothesis of data normality, according to the Shapiro-Wilk test at a", "{100*(1-alfa)}", "% of confidence level."], position, fk_id_function, 2, cursor, connection, fk_id_contributor], # en
        [['Como o valor tabelado (', "{tabulated}", ") > estatística (", "{statistic}", "), nós TEMOS evidências para rejeitar a hipótese de normalidade dos dados, de acordo com o teste de Shapiro-Wilk com", "{100*(1-alfa)}", "% de confiança."], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
    ...
    params = [
        ...
        [["ResultadoShapiroWilk"], position, fk_id_function, 3, cursor, connection, fk_id_contributor], # pt-br
    ]
    for param in params:
        func.insert_into_message_message_parts(*param)
